  ...
  <script type="application/ld+json">
  {
    ...
    "itemListElement": [
      ...
      ${breadcrumbs
        .map(
          (b, i) =>
            `{"@type":"ListItem","position":${i + 2},"name":"${b.name}","item":"${b.url}"}`,
        )
        .join(",")}
    ]
  }
  ...
  const breadcrumbHtml =
    ...
    <ol style="display:flex;flex-wrap:wrap;gap:0.5rem;list-style:none;padding:0;margin:0;font-size:0.875rem;color:#9ca3af;">
      ...
      ${breadcrumbs
        .map(
          (b, i, arr) =>
            `<li style="display:flex;gap:0.5rem;align-items:center;"><span aria-hidden="true">›</span>${
              i === arr.length - 1
                ? `<span style="color:#9ca3af;">${b.name}</span>`
                : `<a href="${b.url}" style="color:#c9a84c;text-decoration:none;">${b.name}</a>`
            }</li>`,
        )
        .join("")}
    </ol>
...
.ssr-highlight-title{font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#c9a84c;margin-bottom:0.5rem}
.ssr-highlight-value{font-family:'Playfair Display',serif;font-size:2rem;font-weight:700;color:#f9f9f7}
.ssr-highlight-sub{font-size:0.875rem;color:#9ca3af;margin-top:0.25rem}
...
.ssr-related h2{font-size:1.25rem;margin-bottom:1rem}
